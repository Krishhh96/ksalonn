...
export class GalleryComponent implements OnInit {
  ...
  imgObjData = [
    ...
    {
      path: '../../assets/images/bleach_thread.jpg',
      name: 'MakeUp',
      details: 'MakeUp & Styling',
    },
    {
      path: '../../assets/images/facial.jpg',
      name: 'Bridal',
      details: 'Bridal MakeUp',
    },
    {
      path: '../../assets/images/engagement.jpg',
      name: 'Bridal',
      details: 'Bridal MakeUp',
    },
  ];
  ...
  ngOnInit(): void {
    // this.imgObj = iterate to length 8 this.imgObj and then assign to this.imgObj
    this.allImages();
    // console.log(this.imgObj);

    // 8th imagediv add innerHTML <div>+more</div>
  }
  allImages() {
    for (let i = 0; i <= this.imgObjData.length - 1; i++) {
      if (i <= 9) {
        this.imgObj[i] = this.imgObjData[i];
      }
    }
  }
  ...
  open(content, imgPath, i) {
    // console.log(i);
    if (i % 9 == 0 && i != 0) {
      this.imgObj = [];
      for (let j = i + 1; j <= this.imgObjData.length - 1; j++) {
        this.imgObj.push(this.imgObjData[j]);
      }
    } else {
